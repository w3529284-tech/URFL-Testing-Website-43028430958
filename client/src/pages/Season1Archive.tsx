...
export default function Season1Archive() {
  const [selectedWeek, setSelectedWeek] = useState(1);
  const [, setLocation] = useLocation();

  const { data: games, isLoading, error } = useQuery<Game[]>({
    queryKey: ["/api/games/week", selectedWeek, "archive-1"],
    queryFn: async () => {
  ...
  const GameCardCompact = ({ game }: { game: Game }) => {
    return (
      <Link href={`/game/${game.id}`}>
        <Card className="p-3 hover-elevate cursor-pointer min-w-44">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-lg mb-6">
          Relive all the action from the inaugural season of URFL
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <p className="text-muted-foreground text-lg">
            No games found for Week {selectedWeek} in Season 1 Archive
          </p>
        </div>
      )}
